...
test('the mark fades out at arm’s length and across the room',()=>{
  const at=head(1.67);
  const peak=d=>settle(fresh(),at,new T.Vector3(0,1.6,d)).peak;
  ...
  // Across a room is where it earns its place.
  const mid=peak(5);
  assert.ok(mid>.5,`too faint at five metres (${mid})`);
  // And it does not carry the length of the silo.
  assert.equal(peak(24),0,'the mark is visible from twenty-four metres');
  // Monotonic through the near fade, so it grows in rather than popping.
  const near=[1.6,2.0,2.4,3.0].map(peak);
