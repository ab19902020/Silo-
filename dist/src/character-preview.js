...
export class CharacterPreview{
 constructor(canvas,status){
  this.canvas=canvas;this.status=status;this.angle=.25;this.walking=false;this.frame=0;this.elapsed=0;this.active=false;
 ...
 tick(now){
  ...
  if(this.actor){this.actor.motion.sample(this.walking?'Walk':'Idle',this.elapsed);this.render();}
 ...
 render(){
  ...
  const distance=height*1.9*Math.max(1,.65/this.camera.aspect);this.camera.position.set(0,height*.59,distance);this.camera.lookAt(0,height*.51,0);this.camera.updateProjectionMatrix();
