...
export function addResidentClothes({a,wide,coat,shirt,dark,surface,add,binding}){
 ...
 const panel=(x,y,w,h,color,offset=.0016)=>{const g=new T.PlaneGeometry(w,h,8,10);g.translate(x,y,0);surface.fit(g,{offset});add(g,color,'Chest',null,true);};
 const button=(x,y,r,color)=>{const g=new T.CircleGeometry(r,16);g.translate(x,y,0);surface.fit(g,{offset:.0030});add(g,color,'Chest');};
 const patch=(x,y,w,h)=>{
 ...
 if(['work','uniform','medical'].includes(a.outfit))for(const s of [-1,1])patch(s*.080*wide,1.277,.070,.086);
 if(!['knit','robe'].includes(a.outfit)){
 ...
 if(!['knit','robe'].includes(a.outfit))for(const sign of [-1,1]){
  const corners=[[.028,1.427],[.050,1.422],[.080,1.382],[.022,1.405]],p=[],idx=[],n=6;
  ...
  const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(p,3));g.setIndex(idx);g.computeVertexNormals();surface.fit(g,{offset:.0018});add(g,shirt,'Chest',null,true);
 }
 if(a.quilted)for(let i=0;i<7;i++)for(const s of [-1,1])panel(s*(.026+i*.017),1.16,.0008,.22,tint(.92));
 if(a.outfit==='vest')for(const s of [-1,1])panel(s*.073*wide,1.21,.117,.28,tint(.61));
 ...
 if(a.outfit==='uniform'){
  const g=new T.CircleGeometry(.015,24);g.scale(1,1.22,1);g.translate(.080*wide,1.29,0);surface.fit(g,{offset:.0036});add(g,new T.Color(0xbca16a),'Chest');
 }
 ...
 if(['coat','cardigan','medical','robe'].includes(a.outfit))for(const [side,sign] of [['L',1],['R',-1]]){
  const hem=a.coatLength??(a.outfit==='robe'?.34:a.outfit==='cardigan'?.67:.59),rows=18,cols=30,p=[],idx=[],si=[],sw=[];
  for(let j=0;j<=rows;j++)for(let i=0;i<=cols;i++){
   const t=j/rows,y=1.008+(hem-1.008)*t,angle=(sign===1?0:Math.PI)+(i/cols)*Math.PI;
   ...
   const flare=(a.outfit==='robe'?.047:.014)*Math.sin(t*Math.PI/2),fold=.0035*Math.sin(angle*10+.7)*Math.pow(t,1.3);
 ...
 if(a.chain){
  ...
  const g=new T.CircleGeometry(.019,24);g.scale(1,1.25,1);g.translate(0,1.258,0);surface.fit(g,{offset:.006});add(g,new T.Color(0xb6a167),'Chest');
 }
 if(a.outfit==='medical'){
