  ...
  const furnishings={
    residential(){
      ...
      for(const side of [-1,1])for(const row of [0,1]){
        ...
        const unitBox=(mat,u,y,v,w,h,d,solid=true)=>box(mat,homeX(u),y,base+v,w,h,d,solid);
        ...
        for(const u of [.84,3.36])unitBox('wood',u,.65,2.2,.16,.75,1.08);
        ...
        table(k,homeX(5.2),base+2.4,1.3,.8);chair(k,homeX(5.2),base+1.55,Math.PI);chair(k,homeX(5.2),base+3.3);
