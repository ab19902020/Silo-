# George's computer — integrated terminal

Claude delivered the DOM-free module and its tests in `2d8f7c8`. Codex
integrated the terminal into George's apartment, the dialog UI and Story saves.
The schematic now describes the implemented mine pressure gallery, crowbar,
Water Filtration kit, isolation wheel and sealing collar. These are declared
game reconstructions. See `AGENT_COORDINATION.md` for ownership.
...
Insert the drive, boot the machine, search for `library`, open the concealed
directory, then open its schematic. Booting without media shows an error;
inserting the drive afterwards still requires a restart. The Back control
returns from a file to its directory and then to the parent directory.
